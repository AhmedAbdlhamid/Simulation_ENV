  ...
  const uint16_t code, const std_msgs::Header & header)
{
  ...
  contact.code = code;
  contact.ident = false;
  contact.posex =modelPose.Pos().X();
  contact.posey =modelPose.Pos().Y();
  contact.posez =modelPose.Pos().Z();
  contact.orinx =modelPose.Rot().X();
  contact.oriny =modelPose.Rot().Y();
  contact.orinz =modelPose.Rot().Z();
  contact.orinw =modelPose.Rot().W();

vis_pub = this->rosnode->advertise<visualization_msgs::Marker>("/visualization_marker_radar",1000);
...
marker.pose.position.x = modelPose.Pos().X();;
marker.pose.position.y = modelPose.Pos().Y();
marker.pose.position.z = (-1*modelPose.Pos().Z());
marker.pose.orientation.x = modelPose.Rot().X();
marker.pose.orientation.y = modelPose.Rot().Y();
marker.pose.orientation.z = (modelPose.Rot().Z());
marker.pose.orientation.w = modelPose.Rot().W();
marker.scale.x = 3; // the length of the box.
marker.scale.y = 3; // the width of the box.
marker.scale.z = 3; // the height of the box.
marker.color.a = 1.0; 
marker.color.r = 0.0;
marker.color.g = 1.0;
marker.color.b = 0.0;
